...
const CONFIDENCE_LABELS = { verified: "Verified", "community-verified": "Community Verified", "user-added": "User Added", "official-source": "Official restaurant source" };
const displayNutrient = (value, unit = "") => value === null || value === undefined ? "Unknown" : `${value}${unit}`;
  ...
  return (
    <section className="trace-feature-surface trace-food-search" style={{ background: "#1f2937", borderRadius: "16px", marginTop: "24px", maxWidth: "700px", padding: "24px", textAlign: "left", width: "100%" }}>
      ...
      <label style={{ display: "block" }}>
        Food search
        <input type="search" placeholder="Search foods by name..." value={query} onChange={(event) => setQuery(event.target.value)} style={{ ...inputStyle, boxSizing: "border-box", fontSize: "18px", marginTop: "8px", maxWidth: "100%", padding: "12px", width: "100%" }} />
      </label>
      {results.length > 0 && (
          ...
          {results.map((food) => (
            <button className="trace-search-result" key={food.id} type="button" onClick={() => onSelectFood(food)} style={{ background: "#111827", border: "1px solid #4b5563", borderRadius: "12px", color: "white", cursor: "pointer", overflowWrap: "anywhere", padding: "14px", textAlign: "left", width: "100%" }}>
              ...
              <span style={{ display: "block", lineHeight: 1.6, marginTop: "6px", overflowWrap: "anywhere" }}>
                {displayNutrient(food.nutrients.calories)} calories · Protein {displayNutrient(food.nutrients.protein, " g")} · Carbohydrates {displayNutrient(food.nutrients.carbohydrates, " g")} · Fat {displayNutrient(food.nutrients.fat, " g")} · Sodium {displayNutrient(food.nutrients.sodium, " mg")}
              </span>
              {food.provenance.completeness === "partial" && <span style={{ color: "#9ca3af", display: "block", fontSize: "13px", marginTop: "6px" }}>Some nutrition values are unavailable because the restaurant does not publish them.</span>}
            </button>
          ))}
        </div>
      )}
      {hasMeaningfulQuery && results.length === 0 && <p style={{ color: "#9ca3af", marginBottom: 0 }}>No catalog foods found. You can enter this food manually below.</p>}
    </section>
  );
}
