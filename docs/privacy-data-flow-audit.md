...
| --- | --- | --- | --- |
| Memories and unfinished draft | `localStorage.memories` stores saved titles, descriptions, dates, categories, and photo references. `localStorage.memoryDraft` stores unfinished Add Memory fields and staged-photo IDs. | Only in a user-created backup | Saved Memories remain until deletion/restore/site clearing. The unfinished draft remains across navigation/restart until save or confirmed discard. Those actions clean its staged-photo state without deleting saved photos. |
...
| Selected photos | IndexedDB `tracePhotoStorage/photos`; photo bytes and memory/workout reference metadata, including durable staged Add Memory photos | Included in a user-created backup. No photo-upload request exists. | Removed when associated photos/records are deleted, when an unfinished Add Memory photo is removed or its draft is confirmed discarded, replaced wholesale on restore, or removed by clearing site data. |
